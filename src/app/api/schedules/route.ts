...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error || !data) return NextResponse.json({ error: "SCHEDULE_WRITE_FAILED" }, { status: 500 });
    try {
      await syncRecurringScheduleWindows(auth.user.id, "/api/schedules:POST", isSyntheticAccount(auth.user.email) ? "synthetic" : "system");
    } catch {
      await auth.supabase.from("recurring_schedules").delete().eq("id", data.id).eq("user_id", auth.user.id);
      return NextResponse.json({ error: "SCHEDULE_SYNC_FAILED" }, { status: 503 });
    }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (error || !data) return NextResponse.json({ error: "SCHEDULE_WRITE_FAILED" }, { status: 500 });
    try {
      await syncRecurringScheduleWindows(auth.user.id, "/api/schedules:PATCH", isSyntheticAccount(auth.user.email) ? "synthetic" : "system");
    } catch {
      await auth.supabase.from("recurring_schedules").update({
        saved_spot_id: existing.saved_spot_id,
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: "SCHEDULE_WRITE_FAILED" }, { status: 500 });
  try {
    await syncRecurringScheduleWindows(auth.user.id, "/api/schedules:DELETE", isSyntheticAccount(auth.user.email) ? "synthetic" : "system");
  } catch {
    return NextResponse.json({ error: "SCHEDULE_SYNC_FAILED" }, { status: 503 });
  }
